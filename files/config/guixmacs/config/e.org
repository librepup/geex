...
This .e.org File is a complete Emacs configuration setup with auto-completion and suggestion, org-mode defaults, snippets, sane keybinds, and a nice theme. In order to use this configuration file you need two things:
 1. This .e.org file inside your $HOME/.guixmacs/config (/home/<USERNAME>/.guixmacs/config/e.org) directory.
...
(let ((orgfile (expand-file-name "~/.guixmacs/config/e.org"))
      ...
      (userorgfile (expand-file-name "~/.guixmacs/config/user.org"))
      (userelfile  (expand-file-name "~/.guixmacs/config/user.el")))
...
** Theme/Colorscheme
#+BEGIN_COMMENT
Set in "~/.guixmacs/config/user.org".
#+END_COMMENT
#+BEGIN_SRC emacs-lisp
(load-theme 'guix t)
#+END_SRC
...
#+END_SRC
** Better Defaults
Set a few sane defaults (copied from https://github.com/johanwiden/exwm-setup).
#+BEGIN_SRC emacs-lisp
  ...
  ;; Do not ask for confirmation about killing processes.
...
#+END_SRC
** Start Server
Automatically start an Emacs Server that can be connected to via 'emacsclient -c'.
#+BEGIN_SRC emacs-lisp
(server-start)
#+END_SRC
** Pipewire
...
#+END_SRC
** Autosaves
Send autosaved files to a specific directory so they don't pollute every working directory you ever exit a file in without saving.
#+BEGIN_SRC emacs-lisp
(make-directory "~/.guixmacs/autosaves" t)
(setq auto-save-file-name-transforms
      `((".*" "~/.guixmacs/autosaves/" t)))
(setq delete-auto-save-files t)
#+END_SRC
** User Information
#+BEGIN_SRC emacs-lisp
(setq user-full-name "Libre Puppy")
(setq user-login-name "puppy")
(setq user-nickname "librepup")
(setq user-mail-address "librepup@member.fsf.org")
#+END_SRC
** Beacon
...
(defun my/insert-wrapfigure ()
  ...
  (insert
   ...
   "  \\includegraphics[width=0.4\\textwidth]{/home/puppy/Pictures/Wallpapers/guix_wp_02.png}\n"
   "\\end{wrapfigure}\n"
   "#+END_LaTeX\n"))
#+END_SRC
...
(setq emms-player-list '(emms-player-mpv)
      emms-info-functions '(emms-info-native
			    emms-info-ogginfo))
(let ((music-dir (expand-file-name "~/Music/")))
  (when (and (file-directory-p music-dir)
             (directory-files music-dir nil directory-files-no-dot-files-regexp))
...
(add-hook 'exwm-init-hook
          (lambda ()
            (let ((exwm-config (expand-file-name "~/.guixmacs/exwm/colemak.el")))
              (when (file-exists-p exwm-config)
                  (load exwm-config)))))
#+END_SRC
** Multiple Cursors
    ...
    (let ((inhibit-read-only t))
      ...
      (let* ((window-width (window-width))
        ...
        (when (display-graphic-p)
          (let ((image (create-image (or (getenv "GUIXMACS_LOGO_PATH") "~/.guixmacs/files/guix_logo_small.png") nil nil :ascent 'center)))
            (when image
